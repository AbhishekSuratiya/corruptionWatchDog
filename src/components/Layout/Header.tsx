      ...
      <header className="bg-white/80 backdrop-blur-xl shadow-lg border-b border-white/20 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-20">
            {/* Logo - Improved spacing and responsive design */}
            ...
            <nav className="hidden lg:flex items-center space-x-2">
              {navigation.map((item) => (
                <Link
                  key={item.name}
                  to={item.href}
                  className={`px-4 py-2 rounded-xl text-sm font-medium transition-all duration-300 transform hover:scale-105 whitespace-nowrap ${
                    isActive(item.href)
                      ? item.href === '/admin'
            ...
            <nav className="hidden md:flex lg:hidden items-center space-x-1">
              ...
              <button
                ...
                className="px-3 py-2 rounded-lg text-xs font-medium text-gray-700 hover:text-red-600 hover:bg-red-50 transition-all duration-300"
              >
                More
              </button>
            </nav>

            {/* Auth Section - Improved responsive design */}
            <div className="flex items-center space-x-3">
              ...
              <button
                onClick={() => setIsMenuOpen(!isMenuOpen)}
                className="md:flex lg:hidden p-2 rounded-xl text-gray-700 hover:text-red-600 hover:bg-red-50/80 backdrop-blur-sm transition-all duration-300 transform hover:scale-105"
              >
                ...
                {!user && (
                  <div className="pt-4 border-t border-gray-200/50 space-y-3">
                    <button
                      ...
                      className="w-full flex items-center justify-center space-x-2 px-4 py-3 rounded-xl text-sm font-medium text-gray-700 hover:text-red-600 hover:bg-red-50/80 transition-all duration-300 border border-gray-200"
                    >
                      <LogIn className="w-4 h-4" />
                      <span>Sign In</span>
                    </button>
