...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Parse request body
    const { userId, banned }: BanUserRequest = await req.json()

    if (!userId || banned === undefined) {
      return new Response(
        JSON.stringify({ error: 'User ID and banned status are required' }),
        { 
          status: 400, 
          headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
        }
    ...
    // Update user metadata to include ban status
    ...
    console.error('Error in admin-ban-user function:', error)
    return new Response(
